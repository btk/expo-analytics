[![npm version](https://badge.fury.io/js/expo-analytic-safe.png)](https://badge.fury.io/js/expo-analytic-safe)

Expo Analytics (Bare Safe)
=========
...
```
npm install expo-analytic-safe --save
```

## Support for web + app projects
Selecting this option when creating a google analytics property the tracking ID it is not prefixed with `'UA-'` but with `G-` resulting in views not showing up. For now use the regular GA property pending resolving issue [#48](https://github.com/ryanvanderpol/expo-analytic-safe/issues/48)

## Breaking Changes in Expo SDK 33

It seems that Expo introduced some breaking changes in SDK 33, so if you are using a version of Expo below 33 please pin your `package.json` to version `1.0.8` of this package.  `expo-analytic-safe` `1.0.9+` is only compatable with Expo SDK 33+.
...
```
import { Analytics, PageHit } from 'expo-analytic-safe';

const analytics = new Analytics('UA-XXXXXX-Y', {name: APP_NAME, slug: APP_SLUG, version: APP_VERSION_STRING});
...
```
import { Analytics, Event } from 'expo-analytic-safe';
...
```
import { Analytics } from 'expo-analytic-safe';
...
```
import { Analytics, Transaction } from 'expo-analytic-safe';
...
```
import { Analytics, AddItem } from 'expo-analytic-safe';
